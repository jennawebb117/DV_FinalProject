...
<br>  
In England, instead of using latitude and longitude, they use a UTM system, Universal Trnasverse Mercator, to produce conformal projections of locations on Earth. Our data set included the Northing and Easting coordinates of the UTM system. Using this we plotted Easting vs Northing coordinates to produce a scatterplot mapping of accidents where each data point represents an accident. Additionally, we applied a quick filter on the number of casualties to be able to determine a variance of casualties in different locations. We then searched for a website that would plot the UTM coordinates on a map in order to compare our scatterplot of accidents to a geographical map of Leeds. 
<h4>Crosstabs:</h4>  
...
<br>    
This crosstab shows the weather conditions vs the lighting conditions by sex of the casualty. The color of each value represents the number of casualties. As one traverses each page, one can see the data represented for each road surface. The data shows that there weren't many casualties when the roads were icy or snowy. Looking at the road surface when dry, most accidents for both males and females occur when the weather was fine without high winds. We can also see that many casualties occured when the surface was wet but still not as many as when the surface was dry. It is also interesting to note that accross all surfaces males had represented higher casualty numbers.
<br>  
